...
def get_all_timetable_slots(week_start: str = None) -> list:
    try:
...
def get_timetable_slots(week_start: str = None) -> list:
    try:
        query = (
        ...
        if week_start:
            result = query.eq("week_start", week_start).execute().data or []
            if result:
                return result
            # Fallback: return all slots if none found for this week
            return (
                supabase.table("timetable_slots")
                .select("*, professors(id, name, title, department)")
                .execute().data or []
